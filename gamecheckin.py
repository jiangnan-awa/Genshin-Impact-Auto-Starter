...
def run_task_selected(game_mids):
    if not game_mids:
        return ""
    games = {
        "honkai2": ("崩坏学园2", Honkai2),
        "honkai3rd": ("崩坏3rd", Honkai3rd),
        "tears_of_themis": ("未定事件簿", TearsOfThemis),
        "genshin": ("原神", Genshin),
        "honkai_sr": ("崩坏：星穹铁道", Honkaisr),
        "zzz": ("绝区零", ZZZ),
    }
    return_data = ''
    for game_mid in game_mids:
        info = games.get(game_mid)
        if not info:
            continue
        game_print_name, game_module = info
        return_data += checkin_game(game_mid, game_module, game_print_name)
    return return_data
