...
def build_page(parent: object, state: Optional[AppState] = None) -> object:
    ...
    def _save_settings_now() -> None:
        try:
            b.update_settings(
                gui_show_cookie_plain=bool(var_show_plain.get()),
                gui_last_selected_account_id=selected_id or "",
            )
        except Exception:
            return
    ...
    var_show_plain = tk.BooleanVar(value=bool(settings.get("gui_show_cookie_plain", False)))
    ...
    def _qr_get_miyoushe_cookie():
        log_action("GUI", "qr_get_miyoushe_cookie", "start")
        def worker(status_cb: Callable[[str], None]) -> Tuple[str, Dict[str, str]]:
            ...
            try:
                ...
                log_action("GUI", "qr_get_miyoushe_cookie", "fail", reason=str(e))
                raise
            try:
    ...
    set_suspend(True)
    try:
        var_show_plain.set(bool(settings.get("gui_show_cookie_plain", False)))
    finally:
        set_suspend(False)
    render_account_list()
    candidate = state.selected_account_id or str(settings.get("gui_last_selected_account_id", "") or "")
