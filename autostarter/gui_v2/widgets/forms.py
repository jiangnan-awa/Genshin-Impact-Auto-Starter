...
def make_button(parent: object, text: str, command=None, **kwargs) -> object:
    """创建统一样式的按钮（固定高度/默认间距由调用方负责 pack/grid）。"""
    ctk = _import_customtkinter()
    # customtkinter 大多控件支持 height 参数；若未来版本不支持，属于运行期问题，不影响 import/测试。
...
def make_browse_file_button(
    ...
    **kwargs,
    ...
    - 内部使用 tkinter.filedialog.askopenfilename
    - 选中后：var.set(path) 并触发 on_selected()

    注意：导入无副作用，因此 tkinter 的导入与对话框调用都在点击回调里完成。
    """

    def _on_click() -> None:
        # 懒加载：避免 import 时产生任何 UI 副作用
...
def make_switch(parent: object, text: str, variable=None, command=None, **kwargs) -> object:
    """创建统一样式的开关（固定高度；variable/command 由调用方传入）。"""
    ctk = _import_customtkinter()
