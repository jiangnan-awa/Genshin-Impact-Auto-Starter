"""
Presets page (GUI v2): “流程编辑器”（Flow Presets）。
...
功能：
- 使用 FlowPresetManager 读写 config/presets.json（flow）
- 显示当前 preset 的 flow（可滚动）
  - 每行提供 上移/下移/删除
  - wait 步骤支持秒数输入框编辑
- 添加步骤：
  - “添加动作步骤”（external_launcher/mod/bettergi/onedragon/genshin_direct）
    已存在的动作步骤在菜单中禁用
  - “添加等待”可多次
- ▶ 一键启动：FlowExecutor.run_flow(preset.flow, account_manager.get_settings())（后台线程 + toast）
- 生成桌面快捷方式：仅 win32 且 sys.frozen（.lnk -> --preset <id>，WorkingDirectory=base_path）
"""
...
def build_page(parent: object, state: Optional[AppState] = None) -> object:
    ...
    def _create_shortcut() -> None:
        ...
        if not is_shortcut_available():
            log_action("GUIv2", "create_shortcut", "skip", preset_id=pid, reason="not_available")
        ...
        # 防止覆盖同名快捷方式导致“名字不对”（例如两个预设同名或曾生成过旧快捷方式）
        # 这里会在已有同名文件时追加短 id。
        ...
        log_action("GUIv2", "create_shortcut", "start", preset_id=pid, steps_count=len(steps))

        def _worker() -> None:
            try:
                # 优先使用 win32com 直接创建快捷方式（最稳），不可用则回退 PowerShell -EncodedCommand
                ...
                log_action("GUIv2", "create_shortcut", "ok", preset_id=pid, file=f"{safe_name}.lnk")
                toast(f"已生成桌面快捷方式：{safe_name}.lnk", 2200)
            except Exception as e:
                log_action("GUIv2", "create_shortcut", "fail", preset_id=pid, reason=str(e))
    ...
    def _run_current_flow() -> None:
        ...
        def _worker() -> None:
            try:
                log_action("GUIv2", "run_preset", "start", preset_id=pid, steps_count=len(steps))
                toast(f"正在执行流程：{name} ...", 1800)
                FlowExecutor.run_flow(steps, account_manager.get_settings())
                log_action("GUIv2", "run_preset", "ok", preset_id=pid, steps_count=len(steps))
                toast(f"已发送启动指令：{name}", 1800)
            except Exception as e:
                log_action("GUIv2", "run_preset", "fail", preset_id=pid, steps_count=len(steps), reason=str(e))
                toast(f"启动失败：{e}", 2500)
    ...
    forms.make_button(left_bar, "设为默认", command=_set_active_preset, width=80).pack(side="left", padx=(0, 8))
    if is_shortcut_available():
        forms.make_button(left_bar, "生成桌面快捷方式", command=_create_shortcut, width=130).pack(side="left")
